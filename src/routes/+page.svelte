<script>
  ...
  export let data;
  const { weather } = data;
  const instantiated = dayjs();

  let refreshCounter = 0;
  let blink = true;
  let toSkip = weather.hourly.time.findIndex((/** @type number */ el) => el * 1000 > date.valueOf()) - 1;
  onMount(() => {
    const interval = setInterval(() => {
      date = new Date();
      // Do a hard reload after midnight
      if (!instantiated.isSame(date, 'day')) {
        window.location.href = '/';
      }
      toSkip = weather.hourly.time.findIndex((/** @type number */ el) => el * 1000 > date.valueOf()) - 1;
      blink = !blink;
      refreshCounter++;
