...
export async function load({ fetch, cookies }) {
  try {
    ...
    const calendar = events.map((el) => {
      ...
      return /** @type App.CalEntry */ ({
        summary: event.summary || '',
        // @ts-ignore ical extends the date object, adding an optional timezone. Ignore the error that there's no TZ on a Date, because there might be.
        start: dayjs.tz(event.start, event?.start?.tz).toISOString() || '',
        // @ts-ignore As above
        end: dayjs.tz(event.end, event?.end?.tz).toISOString() || '',
        location: event.location || '',
        rrule: event.rrule || '',
        // @ts-ignore ical extends the date object, adding an optional 'dateOnly' boolean for all day events.
